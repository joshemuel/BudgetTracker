...
export function Figure({
  ...
  return (
    <div className="py-4">
      ...
      <p className={`num ${color} mt-2 text-3xl md:text-4xl`}>{value}</p>
...
export function SectionTitle({ children, kicker }: { children: ReactNode; kicker?: ReactNode }) {
  return (
    <div className="mb-4">
      ...
      <h2 className="display text-3xl md:text-4xl text-ink">{children}</h2>
...
export function Pullquote({ children }: { children: ReactNode }) {
  return (
    <blockquote className="display-italic text-2xl md:text-3xl text-ink-soft leading-snug border-l-2 border-accent pl-4">
      {children}
    </blockquote>
  );
}
